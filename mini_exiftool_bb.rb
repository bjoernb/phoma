...
class MiniExiftool
  ...
  def self.test
	test_path = "C:/Diskstation/APU/photo/bjoern/03 Nach dem Studium/2011-09-11 Klettern Altmühltal/P1060177.JPG"
	photo = MiniExiftool.new test_path
  end
  ...
  def cmd_gen arg_str='', filename
	# Fix bjoernb force_encoding to UTF-8 removed because of broken file names when German Umlaute
    # [@@cmd, arg_str.encode('UTF-8'), escape(filename.encode(@@fs_enc))].map {|s| s.force_encoding('UTF-8')}.join(' ')
    [@@cmd, arg_str.encode('UTF-8'), escape(filename.encode(@@fs_enc))].join(' ')
  end
